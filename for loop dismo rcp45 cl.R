...
# 2050
#rcp45
land.cover.current <- subset(predictors, c("forest","grassland","farmland","urban"))
...
#predict
for(sp in species.list$Scientific_name[37:60]){
  ...
  for(gcm in c('ac', 'bc','ca','cm','cn','cs','gf','gi','ip','mi')){
    ...
    predictors.2050.rcp45 <- stack(climate.2050.rcp45, land.cover.current, karst)
  ...
  writeRaster(species.2050.rcp45.both.bin, filename = paste0('F:/Working/2018/PhD_research/SDM output R/17_5_23_BC/output_maps/',sp,' 2050 rcp45 cl ',gcm,'.tif'),format='GTiff', overwrite=T)
  print(gcm)
  }
  print(sp)
  }
