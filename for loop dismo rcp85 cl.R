...
for(sp in species.list$Scientific_name){
  ...
  for(gcm in c('ac', 'bc','ca','cm','cn','cs','gf','gi','ip','mi')){
    ...
    predictors.2050.rcp85 <- stack(climate.2050.rcp85, land.cover.current, karst)
    ...
    writeRaster(species.2050.rcp85.both.bin, filename = paste0('F:/Working/2018/PhD_research/SDM output R/17_5_23_BC/output_maps/',sp,' 2050 rcp85 cl ',gcm,'.tif'),format='GTiff', overwrite=T)
    print(gcm)
  }
  print(sp)
}
